...
function svelteChildrenToNodes(source: string, children: any[], warnings: string[]): EditorNode[] {
  ...
  for (const child of children) {
    if (child.type === 'Text') {
      ...
      nodes.push({
        ...
        sourceLocation: offsetToLocation(source, child.start, child.end),
      });
      continue;
    }
    ...
    if (child.type === 'Element' || child.type === 'InlineComponent') {
      ...
      const parsedChildren = svelteChildrenToNodes(source, child.children ?? [], warnings);
...
const svelteAdapter: FrameworkAdapter = {
  ...
  parse(source) {
    ...
    const children = svelteChildrenToNodes(source, ast.html.children ?? [], warnings);
...
function jsxNodeToEditorNode(source: string, node: JSXChildNode, warnings: string[]): EditorNode | null {
  if (t.isJSXSpreadChild(node)) {
    ...
    return {
      ...
      sourceLocation: node.start != null && node.end != null ? offsetToLocation(source, node.start, node.end) : undefined,
    };
  }
  ...
  if (t.isJSXFragment(node)) {
    return {
      ...
      children: node.children
        .map((child) => jsxNodeToEditorNode(source, child, warnings))
...
const reactAdapter: FrameworkAdapter = {
  ...
  parse(source) {
    ...
    const parsedRoot = jsxNodeToEditorNode(source, rootNode, warnings);
