...
const ZIG_ACCELERATOR_ENABLED = import.meta.env.VITE_ENABLE_ZIG_ACCELERATOR === 'true';
const DEFAULT_COPILOT_MODEL = 'moonshotai/kimi-k2-instruct-0905';
...
export default function App() {
  ...
  const [copilotIntent, setCopilotIntent] = useState('Polish the selected node and keep the source structure clean.');
  const [copilotStatus, setCopilotStatus] = useState('Kimi instant mode ready.');
  const [copilotResponse, setCopilotResponse] = useState<CopilotEditResponse | null>(null);
  const [isGeneratingCopilotEdits, setIsGeneratingCopilotEdits] = useState(false);
  const [isApplyingCopilotEdits, setIsApplyingCopilotEdits] = useState(false);
  ...
  async function generateCopilotEdits() {
    if (!document || !copilotIntent.trim()) {
      return;
    }

    setIsGeneratingCopilotEdits(true);
    setCopilotResponse(null);
    setCopilotStatus('Generating edit suggestions with Kimi...');

    try {
      const request: CopilotEditRequest = {
        framework,
      ...
      const response = await fetch(`${backendUrl}/api/ai/copilot/edits`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      const payload = await response.json();
      const suggestion = payload.data as CopilotEditResponse;
      setCopilotResponse(suggestion);
      setCopilotStatus(`Suggested ${suggestion.edits.length} edit${suggestion.edits.length === 1 ? '' : 's'}.`);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown copilot error';
      setCopilotStatus(`Copilot failed: ${message}`);
    } finally {
      setIsGeneratingCopilotEdits(false);
    }
  }

  async function applyCopilotEdits() {
    if (!document || !copilotResponse?.edits.length) {
      return;
    }

    setIsApplyingCopilotEdits(true);
    setCopilotStatus('Applying suggested edits...');

    try {
      const engine = await loadFrameworkEngine();
      let nextDocument = document;
      for (const edit of copilotResponse.edits) {
        nextDocument = engine.applyEdit(nextDocument, edit);
      }

      updateDocument(nextDocument);
      setCopilotStatus(`Applied ${copilotResponse.edits.length} edit${copilotResponse.edits.length === 1 ? '' : 's'}.`);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown apply error';
      setCopilotStatus(`Apply failed: ${message}`);
    } finally {
      setIsApplyingCopilotEdits(false);
    }
  }
            ...
            <div className="inspector-stack">
              ...
              <div className="copilot-box">
                <div className="panel-header">
                  <div>
                    <h3>Kimi Copilot</h3>
                    <p>Backend-routed edit suggestions from NVIDIA NIM.</p>
                  </div>
                  <div className="copilot-meta">
                    <span className="mode-chip">Instant</span>
                    <span className="mode-chip">{ZIG_ACCELERATOR_ENABLED ? 'Zig/WASM requested' : 'TypeScript default'}</span>
                  </div>
                </div>
                ...
                <div className="button-row">
                  <button
                    ...
                    disabled={isGeneratingCopilotEdits || !document || !copilotIntent.trim()}
                    onClick={() => void generateCopilotEdits()}
                    type="button"
                  >
                    {isGeneratingCopilotEdits ? 'Generating...' : 'Generate edits'}
                  </button>
                  <button
                    className="secondary-button"
                    disabled={isApplyingCopilotEdits || !copilotResponse?.edits.length}
                    onClick={() => void applyCopilotEdits()}
                    type="button"
                  >
                    {isApplyingCopilotEdits ? 'Applying...' : 'Apply suggested edits'}
                  </button>
                </div>

                <p className="status-line">{copilotStatus}</p>

                {copilotResponse ? (
                  <div className="copilot-response">
                    <p>
                      <strong>{copilotResponse.model}</strong> via {copilotResponse.provider}
                    </p>
                    <p>{copilotResponse.message}</p>
                    <p>{copilotResponse.edits.length} edit suggestion(s) ready.</p>
                    {copilotResponse.warnings.length ? (
                      <ul>
                        {copilotResponse.warnings.map((warning) => (
                          <li key={warning}>{warning}</li>
                        ))}
                      </ul>
                    ) : null}
...
async function readBackendError(response: Response) {
  try {
    const payload = await response.json();
    return payload.errors?.detail ?? `Request failed with ${response.status}`;
  } catch {
